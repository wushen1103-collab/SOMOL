...
mkdir -p "$ROOT/logs" "$ROOT/results/gauge_calibration"

"$PY" "$ROOT/scripts/tune_gauge_calibration.py" \
  --baseline-predictions "$ROOT/results/transport_baselines/transport_baseline_predictions.parquet" \
  --output-dir "$ROOT/results/gauge_calibration" \
  ...
  2>&1 | tee "$ROOT/logs/gauge_calibration.log"
